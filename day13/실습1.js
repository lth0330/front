...
let max =0;
const words = ['apple', 'banana', 'kiwi', 'strdasdada']
function findLongestWord(longword){
 for(let index = 0; index <= longword.length-1;index++){
    if(longword[index].length > max ){
      max = longword[index].length
    }
...
function findTopStudent(){
  for(let index = 1; index <= students.length-1; index++){
   if(topp.score < students[index].score){
    topp = students[index]
...
console.log(findTopStudent().name)
